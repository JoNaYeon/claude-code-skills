...
- **규칙 위치**: 비공개 조직 정책 레포로 이관됨 (이 레포에는 없음)
- **레포 판별**: 레포 루트 `governance.yaml` 존재, 패키지 `com.seemedi.*`, 제품 접두 `ecg-`/`edo-`/`hel-`/`tlk-`
- **특징(축) → 규칙** (규칙 문서는 모두 비공개 조직 레포에 있음):

| # | 특징 | 왜 특이한가 | 규칙 문서 |
|---|------|-------------|-----------|
| ① | 환자 데이터(PHI) | 보호대상 건강정보, 병원망 비반출 | `patient-data-phi` |
| ② | DB/망 분리 | 식별 온프레미스 / 운영 AWS, 하이브리드·HSM | `db-separation` |
| ③ | 규제(의료기기) | IEC 62304, governance.yaml, 규제 레포 격리 | `regulated-track` |
| ④ | FHIR 의료용어 | FHIR R4/KR Core, finding/order/report | `medical-terminology-fhir` |
| — | `seemedi` 네임스페이스·제품 | 회사 고유 식별자 | `seemedi-namespace` |

> 새 조직이 생기면 이 레지스트리에 `### <조직>` 절을 추가하고 조직별 비공개 정책 레포를 둔다.
...
- 이 레포의 규칙 영역은 `general/`만 두고, 조직 정책은 조직별 **비공개 정책 레포**로 물리 분리한다.
- 재사용 시 `general/`만 복사하면 조직 의존 없이 다른 프로젝트에 이식 가능해야 한다.

---
> **개정 이력**
> - 2026-07-14: "General vs SEEMEDI 특이" 단일 분류에서 **조직 특징 분류·규칙 체계**(조직 레지스트리 + 특징→규칙 매핑)로 개정 (세션 승인).
> - 2026-08-09: 조직 정책의 비공개 조직 레포 이관 반영 — C2 규칙 위치·C4 폴더 분리를 조직 정책 레포 체계로 개정 (세션 승인).

...
| [`/study_memo`](./study/study_memo/SKILL.md) | 파일·링크·텍스트·일상 메모를 Notion MEMO DB에 속성(idea/news/book/note[일상 리스트]/work/ect)별 맞춤 템플릿으로 정리 |
...
문서 작성·버전 관리 형식 규칙을 **General / 조직 정책** 으로 분리. [상세 안내](./_template/_conventions/README.md)
...
| 조직 정책 | 비공개 조직 정책 레포로 이관됨 |

---

## 📏 코드 작성 컨벤션 (`code-conventions/`)

코드 작성 방식을 **General(재사용 가능) / 조직 정책** 으로 분리. [상세 안내](./code-conventions/README.md) · 분류 SSOT [`_init/02`](./_init/02_classification.md)

**분리 기준**: General = 조직 무관, 다른 회사/프로젝트에도 그대로 쓸 수 있는 것(코드 작성법·AWS 세팅·Git·API). 조직 정책 = 특정 조직의 도메인·규제·인프라 때문에만 존재하는 것(민감 데이터 보호·망 분리·규제·도메인 용어).

**레포 체계**: General 컨벤션의 SSOT는 전사 컨벤션 레포(비공개)이며 이 레포의 사본은 참조용이다(상충 시 SSOT 우선). 조직 특이 정책은 비공개 조직 정책 레포에서 관리하며, 이 레포(개인)는 개인 스킬·템플릿과 General 사본만 담는다.
...
### 조직 정책
비공개 조직 정책 레포로 이관됨 — 해당 조직 레포에서 작업할 때만 추가 적용.
...
│   │   └── general/             #     재사용 가능 (조직 정책은 비공개 조직 레포로 이관)
│   └── skill-name/              #   스킬 보일러플레이트
├── code-conventions/            # 코드 작성 컨벤션
│   └── general/                 #   재사용 (by-task 8 · by-language 5, SSOT 사본)

...
형식: `<type>/<이슈번호>-<간단한 설명-슬러그>`

브랜치 종류:
- `feature/<이슈번호>-<슬러그>` — 각종 기능 개발
- `bugfix/<이슈번호>-<슬러그>` — 배포 전 QA에서 발견된 버그 수정
- `release/<버전>` — QA 완료 후 배포 준비
- `hotfix/<이슈번호>-<슬러그>` — 배포 후 긴급 버그 수정
- `develop` — 다음 버전 통합 브랜치 (단독으로 사용)
- `main` — 배포 가능 코드 (단독으로 사용)

이슈 번호가 있으면 반드시 포함합니다. 슬러그는 소문자 영문과 하이픈만 사용합니다.

예시: `feature/12-user-auth`, `bugfix/15-login-token-error`, `hotfix/18-payment-crash`

브랜치 이름은 작업 내용과 이슈 번호를 기반으로 자동 제안하되, 사용자가 수정할 수 있게 합니다.
